...
object CSVUtils {
     ...
     * in one cell.
     */
    fun escapeThenJoin(data: List<String?>): String {
        return data.map { StringEscapeUtils.escapeCsv(it) }.joinToString(",")
    }
    ...
    fun intListToString(data: List<Int>): String {
        return if (data.size == 1) {
            ...
            "\"" + java.lang.String.join(", ", data.stream().map { obj: Int -> obj.toString() }.toList()) + "\""
        }
    }
}
